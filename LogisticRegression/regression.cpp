...
std::vector<double> Regression::calculateGradient(Data &d, double *loss)
{
...
void Regression::doGradientDescent(std::vector<double> &gradVec)
{
...
void Regression::doGradientDescentData(Data &d, double *l)
{

    // std::vector<double> gradVecSum;
    std::vector<double> gradVec;
    // double loss = 0;
    gradVec = Regression::calculateGradient(d, l);
...
void Regression::doGradientDescentData(std::vector<Data> &d, double *l)
{
...
void Regression::train(int steps)
{
    ...
    for (int i = 0; i < steps; i++)
    {
        *loss_epoch = 0.0 ;
        *loss_data = 0.0 ;
        std::vector<Data>::iterator data_iter = Regression::dataset.begin();
        while (data_iter != Regression::dataset.end())
        {
            *loss_data = 0;
            Regression::doGradientDescentData(*data_iter, loss_data);
